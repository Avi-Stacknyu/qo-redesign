...
export const aiAgentModels = pgTable(
  'ai_agent_models',
  {
    ...
    orModelId: text('or_model_id'),
    orSyncedAt: timestamp('or_synced_at', { withTimezone: true, mode: 'string' }),
    orRaw: jsonb('or_raw'),
  ...
  (table) => [
    ...
    uniqueIndex('idx_ai_agent_models_or_model_id')
      .using('btree', table.orModelId.asc().nullsLast().op('text_ops'))
      .where(sql`(or_model_id IS NOT NULL)`),
    foreignKey({
...
export const sessions = pgTable(
  ...
  (table) => [
    index('idx_sessions_user_id').using('btree', table.userId.asc().nullsLast()),
    uniqueIndex('idx_sessions_token').using('btree', table.token.asc().nullsLast()),
    foreignKey({
...
export const accounts = pgTable(
  ...
  (table) => [
    index('idx_accounts_user_id').using('btree', table.userId.asc().nullsLast()),
    foreignKey({
      columns: [table.userId],
      foreignColumns: [users.id],
...
// ============================================================================
// Graph Memory Tables (migrated from Cloudflare Durable Object SQLite)
// ============================================================================
...
export const graphNodes = pgTable(
  'graph_nodes',
  {
    ...
    searchText: text('search_text').generatedAlwaysAs(
      sql`coalesce(data->>'text', data->>'summary', data->>'name', '')`
    ),
    searchCtx: text('search_ctx').generatedAlwaysAs(
      sql`coalesce(data->>'context', data->>'category', '')`
    ),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' }).notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'string' }).notNull()
  },
  (table) => [
    index('idx_graph_nodes_user_id').using('btree', table.userId),
    index('idx_graph_nodes_user_type').using('btree', table.userId, table.type),
    index('idx_graph_nodes_type').using('btree', table.type),
    index('idx_graph_nodes_updated').using('btree', table.updatedAt),
    index('idx_graph_nodes_search').using(
      'gin',
      sql`to_tsvector('english', coalesce(${table.searchText}, '') || ' ' || coalesce(${table.searchCtx}, ''))`
    ),
    index('idx_graph_nodes_data').using('gin', table.data),
    foreignKey({
      columns: [table.userId],
      foreignColumns: [users.id],
      name: 'fk_graph_nodes_user_id'
    }).onDelete('cascade')
  ]
);
...
export const graphEdges = pgTable(
  ...
  (table) => [
    unique('uq_graph_edges_src_tgt_rel').on(
      table.userId,
      table.source,
      table.target,
      table.relationship
    ),
    index('idx_graph_edges_user_source').using('btree', table.userId, table.source),
    index('idx_graph_edges_user_target').using('btree', table.userId, table.target),
    index('idx_graph_edges_relationship').using('btree', table.relationship),
    foreignKey({
      columns: [table.userId],
      foreignColumns: [users.id],
      name: 'fk_graph_edges_user_id'
    }).onDelete('cascade'),
    foreignKey({
...
export const graphDocumentChunks = pgTable(
  'graph_document_chunks',
  {
    id: text().primaryKey().notNull(),
    userId: text('user_id').notNull(),
  ...
  (table) => [
    index('idx_graph_chunks_user_file').using('btree', table.userId, table.fileKey),
    index('idx_graph_chunks_search').using(
      'gin',
      sql`to_tsvector('english', ${table.textContent} || ' ' || coalesce(${table.context}, ''))`
    ),
    foreignKey({
      columns: [table.userId],
      foreignColumns: [users.id],
      name: 'fk_graph_chunks_user_id'
    }).onDelete('cascade')
  ]
);

export const configOnboardingProfiles = pgTable(
  'config_onboarding_profiles',
  {
    id: text().primaryKey().notNull(),
    key: text().notNull(),
    name: text().notNull(),
    ...
    sessionTimeoutMs: numeric('session_timeout_ms'),
    cacheTtlMs: numeric('cache_ttl_ms'),
    enableTrialActivation: boolean('enable_trial_activation').default(true),
    defaultTags: jsonb('default_tags'),
    visibility: text().default('public'),
    disclosures: jsonb(),
    aiFallbackQuestions: jsonb('ai_fallback_questions'),
    created: timestamp({ withTimezone: true, mode: 'string' }),
    updated: timestamp({ withTimezone: true, mode: 'string' })
  },
  (table) => [
    uniqueIndex('idx_onboarding_profiles_key').on(table.key),
    index('idx_onboarding_profiles_status').using(
      'btree',
      table.status.asc().nullsLast().op('text_ops')
    ),
    foreignKey({
      columns: [table.model],
      foreignColumns: [aiAgentModels.id],
      name: 'fk_onboarding_profiles_model'
    }).onDelete('set null'),
    foreignKey({
      columns: [table.systemPrompt],
      foreignColumns: [aiPrompts.id],
      name: 'fk_onboarding_profiles_system_prompt'
    }).onDelete('set null')
  ]
);

export const configOnboardingProfileQuestions = pgTable(
  'config_onboarding_profile_questions',
  {
    id: text().primaryKey().notNull(),
  ...
  (table) => [
    index('idx_onboarding_profile_questions_profile_order').using(
      'btree',
      table.profile.asc().nullsLast().op('text_ops'),
      table.order.asc().nullsLast().op('numeric_ops')
    ),
    foreignKey({
      columns: [table.profile],
      foreignColumns: [configOnboardingProfiles.id],
      name: 'fk_onboarding_profile_questions_profile'
    }).onDelete('cascade')
  ]
);

export const configOnboardingCampaigns = pgTable(
  'config_onboarding_campaigns',
  {
    id: text().primaryKey().notNull(),
    slug: text().notNull(),
    ...
    defaultProfile: text('default_profile').notNull(),
  ...
  (table) => [
    uniqueIndex('idx_onboarding_campaigns_slug').on(table.slug),
    foreignKey({
      columns: [table.defaultProfile],
      foreignColumns: [configOnboardingProfiles.id],
      name: 'fk_onboarding_campaigns_profile'
    })
  ]
);

export const configOnboardingInviteCodes = pgTable(
  'config_onboarding_invite_codes',
  {
    id: text().primaryKey().notNull(),
    code: text().notNull(),
  ...
  (table) => [
    uniqueIndex('idx_onboarding_invite_codes_code').on(table.code),
    foreignKey({
      columns: [table.campaign],
      foreignColumns: [configOnboardingCampaigns.id],
      name: 'fk_onboarding_invite_codes_campaign'
    })
  ]
);

export const userOnboardingAssignments = pgTable(
  ...
  (table) => [
    index('idx_user_onboarding_assignments_user').using(
      'btree',
      table.user.asc().nullsLast().op('text_ops')
    ),
    foreignKey({
      columns: [table.profile],
      foreignColumns: [configOnboardingProfiles.id],
      name: 'fk_user_onboarding_assignments_profile'
    })
  ]
);

export const userOnboardingAuditEvents = pgTable(
  'user_onboarding_audit_events',
  {
    id: text().primaryKey().notNull(),
    user: text().notNull(),
    email: text(),
  ...
  (table) => [
    index('idx_oboa_user').using('btree', table.user.asc().nullsLast().op('text_ops')),
    index('idx_oboa_assignment').using('btree', table.assignment.asc().nullsLast().op('text_ops')),
    index('idx_oboa_profile').using('btree', table.profile.asc().nullsLast().op('text_ops')),
    index('idx_oboa_event_type').using('btree', table.eventType.asc().nullsLast().op('text_ops')),
    index('idx_oboa_created').using('btree', table.created.asc().nullsLast()),
    foreignKey({
      columns: [table.user],
      foreignColumns: [users.id],
      name: 'fk_oboa_user'
    ...
    foreignKey({
      columns: [table.profile],
      foreignColumns: [configOnboardingProfiles.id],
      name: 'fk_oboa_profile'
    }).onDelete('set null'),
    foreignKey({
      columns: [table.assignment],
      foreignColumns: [userOnboardingAssignments.id],
      name: 'fk_oboa_assignment'
    }).onDelete('set null')
  ]
);

// ── Profiler extraction pipeline tables ──────────────────────────────────
export {
  profilerExtractionLog,
  profilerModelChain,
  profilerSchemaProposals
} from './profiler-tables';
