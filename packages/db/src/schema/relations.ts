...
export const profilerAgentsRelations = relations(profilerAgents, ({ one, many }) => ({
  ...
  modelChain: many(profilerModelChain),
  extractionLogs: many(profilerExtractionLog)
}));
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  onboardingAuditEvents: many(userOnboardingAuditEvents)
}));
...
export const planPaymentTransactionsRelations = relations(
  planPaymentTransactions,
  ({ one, many }) => ({
    planPackage: one(planPackages, {
...
export const aiAgentFlowsKnowledgeFilesRelations = relations(
  aiAgentFlowsKnowledgeFiles,
  ({ one }) => ({
    aiAgentFlow: one(aiAgentFlows, {
...
export const aiAgentModelsSupportedToolsRelations = relations(
  aiAgentModelsSupportedTools,
  ({ one }) => ({
    aiAgentModel: one(aiAgentModels, {
...
export const planPackagesAllowedModelsRelations = relations(
  planPackagesAllowedModels,
  ({ one }) => ({
    planPackage: one(planPackages, {
...
export const graphNodesRelations = relations(graphNodes, ({ one, many }) => ({
  ...
  outgoingEdges: many(graphEdges, { relationName: 'edgeSource' }),
  incomingEdges: many(graphEdges, { relationName: 'edgeTarget' })
}));
...
export const graphEdgesRelations = relations(graphEdges, ({ one }) => ({
  ...
  sourceNode: one(graphNodes, {
    fields: [graphEdges.source],
    references: [graphNodes.id],
    relationName: 'edgeSource'
  }),
  targetNode: one(graphNodes, {
    fields: [graphEdges.target],
    references: [graphNodes.id],
    relationName: 'edgeTarget'
  })
}));

export const graphDocumentChunksRelations = relations(graphDocumentChunks, ({ one }) => ({
  user: one(users, {
    fields: [graphDocumentChunks.userId],
    references: [users.id]
  })
}));

export const configOnboardingProfilesRelations = relations(
  configOnboardingProfiles,
  ({ one, many }) => ({
    modelRef: one(aiAgentModels, {
      fields: [configOnboardingProfiles.model],
      references: [aiAgentModels.id]
    }),
    promptRef: one(aiPrompts, {
      fields: [configOnboardingProfiles.systemPrompt],
      references: [aiPrompts.id]
    }),
    questions: many(configOnboardingProfileQuestions),
    campaigns: many(configOnboardingCampaigns),
    assignments: many(userOnboardingAssignments),
    auditEvents: many(userOnboardingAuditEvents)
  })
);

export const configOnboardingProfileQuestionsRelations = relations(
  configOnboardingProfileQuestions,
  ({ one }) => ({
    profileRef: one(configOnboardingProfiles, {
      fields: [configOnboardingProfileQuestions.profile],
      references: [configOnboardingProfiles.id]
    })
  })
);

export const configOnboardingCampaignsRelations = relations(
  configOnboardingCampaigns,
  ({ one, many }) => ({
    profileRef: one(configOnboardingProfiles, {
      fields: [configOnboardingCampaigns.defaultProfile],
      references: [configOnboardingProfiles.id]
    }),
    inviteCodes: many(configOnboardingInviteCodes)
  })
);

export const configOnboardingInviteCodesRelations = relations(
  configOnboardingInviteCodes,
  ({ one }) => ({
    campaignRef: one(configOnboardingCampaigns, {
      fields: [configOnboardingInviteCodes.campaign],
      references: [configOnboardingCampaigns.id]
    })
  })
);

export const userOnboardingAssignmentsRelations = relations(
  userOnboardingAssignments,
  ({ one, many }) => ({
    profileRef: one(configOnboardingProfiles, {
      fields: [userOnboardingAssignments.profile],
      references: [configOnboardingProfiles.id]
    }),
    auditEvents: many(userOnboardingAuditEvents)
  })
...
export const userOnboardingAuditEventsRelations = relations(
  userOnboardingAuditEvents,
  ({ one }) => ({
    userRef: one(users, {
      fields: [userOnboardingAuditEvents.user],
      references: [users.id]
    }),
    profileRef: one(configOnboardingProfiles, {
      fields: [userOnboardingAuditEvents.profile],
      references: [configOnboardingProfiles.id]
    }),
    assignmentRef: one(userOnboardingAssignments, {
      fields: [userOnboardingAuditEvents.assignment],
      references: [userOnboardingAssignments.id]
    })
  })
);

export const profilerExtractionLogRelations = relations(profilerExtractionLog, ({ one }) => ({
  profilerAgent: one(profilerAgents, {
    fields: [profilerExtractionLog.profilerAgentId],
    references: [profilerAgents.id]
  })
}));

export const profilerModelChainRelations = relations(profilerModelChain, ({ one }) => ({
  profilerAgent: one(profilerAgents, {
    fields: [profilerModelChain.profilerAgentId],
    references: [profilerAgents.id]
  }),
  aiAgentModel: one(aiAgentModels, {
    fields: [profilerModelChain.modelId],
    references: [aiAgentModels.id]
  })
}));

export const profilerSchemaProposalsRelations = relations(profilerSchemaProposals, ({ one }) => ({
  extractionLog: one(profilerExtractionLog, {
    fields: [profilerSchemaProposals.extractionLogId],
    references: [profilerExtractionLog.id]
  })
}));
