...
const authentication: Handle = async ({ event, resolve }) => {
  let client: Awaited<ReturnType<typeof getDb>>['client'] | undefined;
  try {
    // Initialize database via Hyperdrive
    const connection = await getDb(event.platform!);
    const { db } = connection;
    ...
    await client?.end().catch((err: unknown) => {
      ...
      if (code === '57P01' || code === '57P02' || code === '57P03') return;
      console.warn('[hooks:auth] failed to close db client', err);
    });
  }
};
