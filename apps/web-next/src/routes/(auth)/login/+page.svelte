...
<div class="space-y-5">
	...
	{#if authMethods?.email && otpMode === 'idle'}
		...
		>
			...
			<div>
				<div class="mb-1 flex items-center justify-between">
					...
					<a
						...
						class="text-sm font-medium text-violet-700 hover:text-violet-800 hover:underline"
					>
						Forgot password?
					</a>
				</div>
			...
			<button
				type="submit"
				class="flex h-11 w-full items-center justify-center gap-x-2 rounded-full bg-[#7c3aed] px-6 text-base font-semibold text-white shadow-md transition-all duration-200 hover:bg-[#6d28d9] hover:shadow-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-700 active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
	...
	{#if otpMode === 'request' && authMethods?.otp}
		...
		<form class="space-y-5" {...requestOTP}>
			...
			<button
				type="submit"
				class="flex h-12 w-full items-center justify-center gap-x-2 rounded-full bg-[#7c3aed] px-6 text-sm font-bold text-white shadow-md transition-all duration-200 hover:bg-[#6d28d9] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-700 active:scale-[0.98] disabled:opacity-70"
	...
	{#if otpMode === 'idle'}
		...
		<p class="mt-8 text-center text-sm text-slate-600">
			...
			<a href="/register" class="font-bold text-[#7c3aed] hover:text-[#6d28d9] hover:underline">
				Register here!
			</a>
		</p>
	{/if}
