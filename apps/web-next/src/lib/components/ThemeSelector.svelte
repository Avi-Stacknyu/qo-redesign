<script lang="ts">
	...
	const activePresetInfo = $derived(
		THEME_PRESET_LIST.find((preset) => preset.id === currentPreset) ?? THEME_PRESET_LIST[0]
	);
</script>
...
<Popover.Root bind:open>
	<Popover.Trigger>
		{#snippet child({ props })}
			...
			{:else}
				<Button {...props} variant="outline" size="sm" class="gap-2 rounded-2xl">
	...
	<Popover.Content
		align="end"
		class="w-64 rounded-3xl border border-border/60 bg-card/95 p-3 shadow-xl backdrop-blur"
	>
