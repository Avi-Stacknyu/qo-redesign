<script lang="ts">
	...
	const navItems = $derived(
		[
			{ key: 'dashboard', label: 'Dashboard', icon: LayoutGrid, path: '/', show: true },
			{
				key: 'knowledge',
				label: 'Knowledge',
			...
			},
			{
				...
				icon: Users,
				path: '/family-office',
				show: showFeature('page:family-office')
			}
...
<Tooltip.Provider delayDuration={150}>
	<main class="fixed top-[12.5%] left-4 z-30 flex w-fit flex-col gap-5">
		<Card.Root class="w-fit rounded-full border-white/80 bg-white/85 p-2 shadow-sm backdrop-blur-xl">
	...
	<Card.Root class="w-fit rounded-full border-white/80 bg-white/85 p-2 shadow-sm backdrop-blur-xl">
		<Card.Content class="flex flex-col gap-3 p-1">
			...
			<Tooltip.Root delayDuration={150}>
				...
				<Tooltip.Content side="right">{user?.name ?? user?.email ?? 'Account'}</Tooltip.Content>
			</Tooltip.Root>
		</Card.Content>
		</Card.Root>
	</main>
