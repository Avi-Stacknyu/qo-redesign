/**
 * Server-side theme resolution — zero FOUC.
 *
 * Reads the `qo-theme` cookie, looks up a preset, and generates inline CSS
 * variables that get injected into `<html>` via `transformPageChunk` in hooks.
 */
...
/** Default theme when no cookie is set */
export const DEFAULT_THEME: ThemeCookie = { preset: 'orion', mode: 'dark' };

// ── Resolution ───────────────────────────────────────────────────────────────

/**
 * Parse the `qo-theme` cookie value into a ThemeCookie.
 * Falls back to DEFAULT_THEME on any parse error.
 */
export function parseThemeCookie(raw: string | undefined): ThemeCookie {
	...
	try {
		...
		if (
			...
			// Accept any preset name — client-side tweakcn presets won't exist
			// in the server map, but we still honour the mode (dark/light).
			// If preset isn't in THEME_PRESETS, resolveTheme will use default vars.
			return parsed as ThemeCookie;
		}
	} catch {
...
/**
 * Resolve a ThemeCookie into the full set of CSS variables and metadata
 * needed for injection into the HTML.
 */
export function resolveTheme(cookie: ThemeCookie): ResolvedTheme {
	const preset = THEME_PRESETS[cookie.preset] ?? THEME_PRESETS[DEFAULT_THEME.preset]!;
	const vars = preset[cookie.mode];
	const cssInline = buildThemeCSSInline(vars);
...
/**
 * Build an inline `style` string from theme variables.
 * Output: `--background:oklch(...);--foreground:oklch(...);...`
 */
function buildThemeCSSInline(vars: ThemeVars): string {
