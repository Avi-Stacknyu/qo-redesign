			...
			<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
				...
				>
					<input
						...
						required
						class="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
					/>
					...
					<div class="grid gap-3 sm:grid-cols-2">
						{#each data.profiles as profile (profile.id)}
							<form method="POST" action="?/chooseProfile" use:enhance>
								...
								<button
									type="submit"
									class="w-full cursor-pointer rounded-xl border border-slate-200 bg-white p-5 text-left shadow-sm transition-all hover:border-emerald-200 hover:shadow-md hover:shadow-emerald-100/50"
								>
