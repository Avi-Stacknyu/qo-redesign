<script lang="ts">
	...
	import { deleteTool } from './tool.remote';
	...
	async function handleDelete() {
		deleting = true;
		try {
			const result = await deleteTool({ id: row.original.id });
			if (result?.success) {
			...
			toast.error((e as Error).message || 'Failed to delete tool');
		} finally {
			deleting = false;
		}
	}
	...
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				This will permanently delete this tool and remove it from available configurations.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
