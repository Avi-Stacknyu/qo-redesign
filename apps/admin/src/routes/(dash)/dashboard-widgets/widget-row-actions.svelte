<script lang="ts">
	...
	import { deleteWidget } from './widget.remote';
	...
	async function handleDelete() {
		deleting = true;
		try {
			const result = await deleteWidget({ id: row.original.id });
			if (result?.success) {
			...
			toast.error((e as Error).message || 'Failed to delete widget');
		} finally {
			deleting = false;
		}
	}
	...
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				This will remove the widget from the catalog. Existing user instances will not be affected.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
